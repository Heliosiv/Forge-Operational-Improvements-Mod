export function createGatherSettingsAccess({
  ...
  function getGatherResourceConfig() {
    ...
    return {
      ...
      minimumHours: clampGatherInteger(
        getSetting(settings.GATHER_MIN_HOURS, gatherDefaults.minimumHours),
        1,
        24,
        gatherDefaults.minimumHours
      ),
      disallowCombat: Boolean(getSetting(settings.GATHER_DISALLOW_COMBAT, gatherDefaults.disallowCombat)),
      baseDc: {
        lush_forest_or_river_valley: clampGatherInteger(
          getSetting(settings.GATHER_DC_LUSH, gatherDefaults.baseDc.lush_forest_or_river_valley),
          1,
          30,
          gatherDefaults.baseDc.lush_forest_or_river_valley
        ),
        temperate_hills_or_light_woodland: clampGatherInteger(
          getSetting(settings.GATHER_DC_TEMPERATE, gatherDefaults.baseDc.temperate_hills_or_light_woodland),
          1,
          30,
          gatherDefaults.baseDc.temperate_hills_or_light_woodland
        ),
        sparse_plains_or_rocky: clampGatherInteger(
          getSetting(settings.GATHER_DC_SPARSE, gatherDefaults.baseDc.sparse_plains_or_rocky),
          1,
          30,
          gatherDefaults.baseDc.sparse_plains_or_rocky
        ),
        cold_mountains_or_swamp: clampGatherInteger(
          getSetting(settings.GATHER_DC_COLD, gatherDefaults.baseDc.cold_mountains_or_swamp),
          1,
          30,
          gatherDefaults.baseDc.cold_mountains_or_swamp
        ),
        desert_blighted_wasteland: clampGatherInteger(
          getSetting(settings.GATHER_DC_DESERT, gatherDefaults.baseDc.desert_blighted_wasteland),
          1,
          30,
          gatherDefaults.baseDc.desert_blighted_wasteland
        )
      },
      seasonMod: clampGatherModifier(
        getSetting(settings.GATHER_DEFAULT_SEASON_MOD, gatherDefaults.seasonMod),
        gatherDefaults.seasonMod
      ),
      weatherMod: clampGatherModifier(
        getSetting(settings.GATHER_DEFAULT_WEATHER_MOD, gatherDefaults.weatherMod),
        gatherDefaults.weatherMod
      ),
      corruptionMod: clampGatherModifier(
        getSetting(settings.GATHER_DEFAULT_CORRUPTION_MOD, gatherDefaults.corruptionMod),
        gatherDefaults.corruptionMod
      ),
      herbalismAdvantageEnabled: Boolean(
        getSetting(settings.GATHER_ENABLE_HERBALISM_ADVANTAGE, gatherDefaults.herbalismAdvantageEnabled)
      ),
      hostileEncounterFlagEnabled: Boolean(
        getSetting(settings.GATHER_ENABLE_HOSTILE_FAIL_FLAG, gatherDefaults.hostileEncounterFlagEnabled)
      ),
      failBy5ComplicationEnabled: Boolean(
        getSetting(settings.GATHER_ENABLE_FAIL_BY5_COMPLICATION, gatherDefaults.failBy5ComplicationEnabled)
      ),
      successBy5DoubleEnabled: Boolean(
        getSetting(settings.GATHER_ENABLE_SUCCESS_BY5_DOUBLE, gatherDefaults.successBy5DoubleEnabled)
      ),
      nat20BonusEnabled: Boolean(getSetting(settings.GATHER_ENABLE_NAT20_BONUS, gatherDefaults.nat20BonusEnabled)),
      nat1ComplicationEnabled: Boolean(
        getSetting(settings.GATHER_ENABLE_NAT1_FLAG, gatherDefaults.nat1ComplicationEnabled)
      ),
      corruptionWaterCheckEnabled: Boolean(
        getSetting(settings.GATHER_ENABLE_CORRUPTION_WATER_CHECK, gatherDefaults.corruptionWaterCheckEnabled)
      ),
      corruptionConSaveDc: clampGatherInteger(
        getSetting(settings.GATHER_CORRUPTION_SAVE_DC, gatherDefaults.corruptionConSaveDc),
        1,
        30,
        gatherDefaults.corruptionConSaveDc
      ),
      waterAutoFoundEnabled: Boolean(
        getSetting(settings.GATHER_ENABLE_WATER_AUTO_FOUND, gatherDefaults.waterAutoFoundEnabled)
      ),
      travelTradeoffEnabled: Boolean(
        getSetting(settings.GATHER_ENABLE_TRAVEL_TRADEOFF, gatherDefaults.travelTradeoffEnabled)
      ),
      travelTradeoffDefault: normalizeGatherTravelTradeoff(
        getSetting(settings.GATHER_TRAVEL_TRADEOFF_DEFAULT, gatherDefaults.travelTradeoffDefault)
      ),
      travelConSaveDc: clampGatherInteger(
        getSetting(settings.GATHER_TRAVEL_CON_SAVE_DC, gatherDefaults.travelConSaveDc),
        1,
        30,
    ...
    return getGatherQuickPresets(config).map((preset) => {
      ...
      const gatherMode =
        String(options.gatherMode ?? "standard")
          ...
          : "standard";
      const tagParts = [];
      if (options.duringTravel) tagParts.push("Travel");
      if (options.hostileTerrain) tagParts.push("Hostile");
      if (options.isCorruptedRegion) tagParts.push("Corrupted");
      if (resourceType === "water" && options.waterAutoFound) tagParts.push("Auto-water");
      return {
